...
export const Login: React.FC = () => {
  ...
  // Push API feedback errors into user perspective securely
  useEffect(() => {
  ...
  const parseError = useCallback(
    (err: any, context: "login" | "google" | "resend" = "login"): string => {
      if (!navigator.onLine) {
      ...
      if (context === "google") {
        ...
        return serverMsg || "Google sign-in didn't work. Please try again.";
      }
  ...
  useEffect(() => {
    ...
    const renderButton = () => {
      ...
      try {
        ...
        console.error("Google button initialization failed:", e);
      }
    };
  ...
  // Automatically reset contextual error states when user alters inputs
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      if (message.toLowerCase().includes("not verified")) {
        ...
        setApiError(parseError(err, "login"));
      }
    } finally {
    ...
    <div className="min-h-screen relative overflow-hidden font-sans selection:bg-stone-900 selection:text-white flex flex-col">
      ...
      <main className="relative z-10 flex-1 flex flex-col justify-center items-center px-4 sm:px-6 pb-16">
        ...
        >
          ...
          {/* Social Access Control Layer (Google Sign-In Overlaid Interface) */}
          <div className="mb-6">
            <div className="relative w-full h-[52px] rounded-lg overflow-hidden bg-white/95 hover:bg-white border border-white/20 hover:border-sky-300 focus-within:border-sky-400 transition-colors duration-200 flex items-center justify-center shadow-[0_4px_20px_-2px_rgba(0,0,0,0.08)] select-none active:scale-[0.99] group">
              {/* Native Aesthetic Presentation UI */}
              ...
              {/* Secure Invisible Interactive Frame Wrapper */}
              <div
                ref={googleBtnRef}
          ...
          >
            ...
            <div className="flex justify-end">
              <Link
                to="/forgot-password"
                className="text-white/90 hover:text-white text-sm font-medium hover:underline underline-offset-2 transition-colors duration-150"
              >
                Forgot password?
              </Link>
            </div>

            {/* Form Dispatch Action Trigger */}
            <button
              type="submit"
